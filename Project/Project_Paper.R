...
###############################################################################
# Data : We are going to use the dataset sleep!
...
MRM_log <- lm(log(wage) ~ belavg+abvavg+exper+goodhlth+black+female+married+bigcity+smllcity+service+educ,sleep)
...
Confusion_Matrix <- table(df[, "EEA"], predict(LPM_wls) >= 0.4)
prop.table(Confusion_Matrix,1)
(Percent_correctly_predicted_overall <- (sum(y_hat_wls >= 0.4 & df$EEA==1) + sum(y_hat_wls <= 0.4 & df$EEA==0)) / length(df$EEA)) 
